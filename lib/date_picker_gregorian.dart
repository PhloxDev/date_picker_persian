...
/// monthly calendar initially in the dialog shown by calling [showDatePickerShamsi].
///
/// Also see:
///
  ...
  Widget build(BuildContext context) {
    ...
    final Widget yearButton = IgnorePointer(
      ...
      child: _DateHeaderButton(
        ...
        child: Semantics(
            selected: mode == DatePickerMode.year, child: new Container()
//          Text(
//              '${gregorianToJalali(
//                  selectedDate.year, selectedDate.month, selectedDate.day)[0]}'
////              localizations.formatYear(selectedDate)
//              , style: new TextStyle(fontSize: 20.0,
//              fontFamily: 'Tanha', color: Colors.white)
//          ),
            ),
      ),
    );

    var miladiToShamsi = gregorianToJalali(
...
//              getPersianDate(selectedDate)
                '${miladiToShamsi[2]} ${getNameMonthPersian(miladiToShamsi[1])} ${miladiToShamsi[0]}'
//              localizations.formatMediumDate(selectedDate)
                ,
                style: new TextStyle(
                    fontFamily: 'Tanha', fontSize: 20.0, color: Colors.white)),
          ),
        ),
      ),
    );
...
String getNameMonthPersian(int month) {
  if (month == 1) {
    return 'فروردین';
  } else if (month == 2) {
    return 'اردیبهشت';
...
/// [showDatePickerShamsi], which creates a date picker dialog.
///
/// See also:
///
///  * [showDatePickerShamsi].
...
class DayPicker extends StatelessWidget {
  ...
  static const List<int> _daysInMonthShamsi = <int>[
    31,
    31,
    31,
    31,
  ...
  static bool kabiseShamsi(int year) {
    var mod = year % 33;
...
//  **********************************************
  ...
  static int getDaysInMonthShamsi(int year, int month) {
    if (month == 12) {
      return kabiseShamsi(year) ? 30 : 29;
    }
    return _daysInMonthShamsi[month - 1];
  }
  ...
  int _computeFirstDayOffset(
    ...
    var jalaliToShamsi = jalaliToGregorian(year, month, 1, false);

    final int weekdayFromMonday =
        DateTime(jalaliToShamsi[0], jalaliToShamsi[1], jalaliToShamsi[2])
                .weekday -
            1;
  ...
  Widget build(BuildContext context) {
    ...
    var miladiToShamsi = gregorianToJalali(
        displayedMonth.year, displayedMonth.month, displayedMonth.day);

    var selectedDateShamsi = gregorianToJalali(
        selectedDate.year, selectedDate.month, selectedDate.day);

    final int year = miladiToShamsi[0];
    final int month = miladiToShamsi[1];

    final int daysInMonth = getDaysInMonthShamsi(year, month);
    ...
    for (int i = 0; true; i += 1) {
      ...
      if (day < 1) {
        ...
        var dateBuild = jalaliToGregorian(
            miladiToShamsi[0], miladiToShamsi[1], day, false);

        final DateTime dayToBuild =
            DateTime(dateBuild[0], dateBuild[1], dateBuild[2]);
        ...
        final bool isSelectedDay = selectedDateShamsi[0] == miladiToShamsi[0] &&
            selectedDateShamsi[1] == miladiToShamsi[1] &&
            selectedDateShamsi[2] == day;
    ...
    var displayedMonthShamsi = gregorianToJalali(
        displayedMonth.year, displayedMonth.month, displayedMonth.day);
...
//                  localizations.formatMonthYear(displayedMonth),
                  '${getNameMonthPersian(displayedMonthShamsi[1])}'
                      '${displayedMonthShamsi[0]}',
                ),
              ),
            ),
          ),
...
class _MonthPickerState extends State<MonthPicker>
  ...
  static int _monthDelta(DateTime startDate, DateTime endDate) {
    ...
    var startDateShamsi =
        gregorianToJalali(startDate.year, startDate.month, startDate.day);

    var endDateShamsi =
        gregorianToJalali(endDate.year, endDate.month, endDate.day);

    return (endDateShamsi[0] - startDateShamsi[0]) * 12 +
        endDateShamsi[1] -
        startDateShamsi[1];
  }
...
///  * [showTimePicker]
///  * <https://material.google.com/components/pickers.html#pickers-date-pickers>
Future<DateTime> showDatePickerShamsi({
